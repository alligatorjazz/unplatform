...
export const LiteracyLevelSchema = z.preprocess(
  (value) => String(value),
  z.enum(["0", "1", "2", "3", "4"]),
);
...
export const RecommendationSchema = z.object({
  ...
  city: z.union([
    CitySchema.default("Digital First"),
    CitySchema.exclude(["Digital First"]).array(),
  ]),
...
export const BlogPostSchema = z
  .object({
    ...
    externalPub: z.string().optional(),
    externalLink: z.string().optional(),
  })
  .refine(
    ({ externalPub, externalLink }) =>
      !((externalPub && !externalLink) || (!externalPub && externalLink)),
    {
      message:
        "If you're linking an external article, you need both externalPub and externalLink.",
    },
  );
